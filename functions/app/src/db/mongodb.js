...
const getExpenses = async (userId, fromDate, toDate) => {
    const collection = await getDBCollection();
    return await collection.find({ "date": { $gte: fromDate, $lte: toDate }, "userId": { $eq: userId } }).sort({ "date": -1, "createdAt": 1 }).toArray();
}
...
const updateExpense = async (userId, id, category, description, amount, date, updatedAt) => {
    const collection = await getDBCollection();
    const result = await collection.updateOne({ userId, _id: new ObjectId(id) }, { $set: { description, amount, date, category, updatedAt} });
